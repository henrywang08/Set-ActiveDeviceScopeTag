...
function Update-ScopeTagsforApps {
    param (
    ...
    Invoke-MgGraphRequest -Method PATCH `
        -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps/$policyId" `
        -Body $bodyJson `
...
# Connect to Graph
Connect-MgGraph -Scopes "DeviceManagementApps.ReadWrite.All","DeviceManagementRBAC.Read.All"
...
try {
    ...
    $allPolicies = Get-MgBetaDeviceAppManagementMobileApp -All -ErrorAction Stop
} catch {
...
$matchingPolicies = $allPolicies | Where-Object {
    $display = $_.DisplayNameName
    if (-not $display) { $display = $_.DisplayName }
    $display -like $policyNamePattern
}

Write-Host "Found $($matchingPolicies.Count) matching app name(s):"
...
foreach ($policy in $matchingPolicies) {
   #   $policyId = $p.id
    $p = Get-MgBetaDeviceAppManagementMobileApp -MobileAppId $($policy.id) -Property roleScopeTagIds
    $policyId = $policy.Id
    $updatedTags = @($scopeTagId)
    Write-Host "The Scope Tags before change for app '$($p.DisplayName)' ($policyId): $($p.roleScopeTagIds -join ', ')"
    Update-ScopeTagsforApps -updatedTags $updatedTags -policyId $policyId -odatatype $($policy.AdditionalProperties.'@odata.type')
